...
class PeopleMiddleware {
	/**
	 * validates user params with respect to PeopleParams model
	 * @method module:People~PeopleMiddleware.validateUserParams
	...
	static validateUserParams(req, res, next) {
		try {
			...
			logger.error('Error while validating user params in sending people and species data', __filename, '.validateUserParams()', err, 'critical');
			return res.send({
				error: true,
